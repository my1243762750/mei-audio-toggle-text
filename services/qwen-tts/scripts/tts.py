...
def get_model(model_id: str):
    ...
    cache_key = f"_model_{model_id.replace('/', '_').replace('-', '_')}"
    if not hasattr(get_model, cache_key):
        print(f"Loading model: {model_id}")
        setattr(get_model, cache_key, load_model(model_id))
        print("Model loaded!")
        model = getattr(get_model, cache_key)
        if hasattr(model, "get_supported_speakers"):
            print(f"   Supported speakers: {model.get_supported_speakers()}")
    return getattr(get_model, cache_key)
...
def generate_speech(
    ...
    model_id = DEFAULT_MODEL or MODEL_BY_MODE[mode]
    ...
    if mode == "voice_design":
        ...
        if not ref_audio or not ref_text:
            raise ValueError("base_clone mode requires --ref-audio and --ref-text")
...
Examples:
  ...
  python tts.py "其实我真的有发现..." --instruct 冷静分析 --speed 1.1
    ...
    parser.add_argument("--mode", choices=MODEL_BY_MODE.keys(), default="custom_voice",
                        help="TTS mode")
    ...
    parser.add_argument("--ref-audio", default=None,
                        help="Reference audio path for base_clone mode")
    parser.add_argument("--ref-text", default=None,
                        help="Transcript of reference audio for base_clone mode")
